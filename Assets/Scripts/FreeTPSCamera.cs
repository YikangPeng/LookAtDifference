...
public class FreeTPSCamera : MonoBehaviour
{
    ...
    */
    ...
    //通过位置变化计算人物的运动速度
    private Vector3 cameraTargetPosition;//相机目标位置，相机需要平滑运动，需要一个目标位置
    
    /*
    private Vector3 lastPosition;
    private Vector3 currentPosition;
    private Vector3 velocity;
    private float velOffset = 0.0f;
    */
    ...
    void Start()
    {
        ...
        */
        ...
        //lastPosition = CameraPivot.transform.position;
    ...
    bool Inwall()
    {
        ...
        RaycastHit hit;
        LayerMask mask = (1 << LayerMask.NameToLayer("Character")) | (1 << LayerMask.NameToLayer("Ignore Raycast")) | (1 << LayerMask.NameToLayer("Mob")) | (1 << LayerMask.NameToLayer("Weapon")); //将物体的Layer设置为Ignore Raycast,Player和Mob来忽略相机的射线，不然相机将跳到某些物体前,比如怪物,玩家等,
        ...
        {//也就是说，这个if就是指被遮挡的情况
            ...
            wallHitDistance = hit.distance;//碰撞点距离
    ...
    void FreeCamera()
    {
        ...
        float eulerAngle = 90 - Mathf.Acos(Vector3.Dot(offset.normalized, Vector3.up)) * Mathf.Rad2Deg;
        ...
        if (!lockTarget)
        {
            ...
            float assistRotateSpeed = 1.0f;


            if (!lockTarget)
            {
                transform.RotateAround(CameraPivot.transform.position, Vector3.up, rotateSpeed * Input.GetAxis("Mouse X") /*+ assistRotateSpeed * Input.GetAxis("Horizontal")*/);//x不用限制
                offset = Quaternion.AngleAxis(rotateSpeed * Input.GetAxis("Mouse X") /*+ +assistRotateSpeed * Input.GetAxis("Horizontal")*/, Vector3.up) * offset;
            }


            //水平辅助视角，水平移动的时候相机会跟随旋转

            //transform.RotateAround(CameraPivot.transform.position, Vector3.up, assistRotateSpeed * Input.GetAxis("Horizontal"));
            //Debug.Log(rotateSpeed * Input.GetAxis("Mouse X")+"+"+assistRotateSpeed * Input.GetAxis("Horizontal"));
            /*
           ...
           // Debug.Log(eulerX + "+" + eulerAngle);

            Vector3 cameraRight = Vector3.ProjectOnPlane(transform.right, Vector3.up);

            if ((inputY >= 0) && (eulerAngle - rotateSpeed * inputY > -maxEvelation))
            {
                //transform.RotateAround(CameraPivot.transform.position, cameraRight, -rotateSpeed * inputY);//允许滑动
                offset = Quaternion.AngleAxis(-rotateSpeed * inputY, cameraRight) * offset;

                //Debug.Log("UP" + inputY);
            }

            if ((inputY < 0) && (eulerAngle - rotateSpeed * inputY < maxDepression))
            {
                //transform.RotateAround(CameraPivot.transform.position, cameraRight, -rotateSpeed * inputY);
                offset = Quaternion.AngleAxis(-rotateSpeed * inputY, cameraRight) * offset;

                //Debug.Log("DOWN" + inputY);
            }
        ...
        /*
        //加入运动的影响
        currentPosition = CameraPivot.transform.position;
        ...
        Vector3 cameraVelocity = Vector3.ProjectOnPlane(velocity, cameraRight);

        //velOffset += Vector3.Magnitude(cameraVelocity) * 1.0f;
        //if (Vector3.Dot(cameraVelocity, cameraForward)<0)
        velOffset += Vector3.Dot(cameraVelocity, cameraForward.normalized);

        if (Vector3.Magnitude(cameraVelocity) < 0.01f)
            if (Mathf.Abs(velOffset) < 0.1f)
                velOffset = 0;
            else
                //velOffset = velOffset - (velOffset/ Mathf.Abs(velOffset) * 0.1f);
                velOffset *= cameraDamp;

        velOffset = Mathf.Clamp(velOffset, -2.0f, 2.0f);
        */
        ...
        //transform.position = Vector3.SmoothDamp(transform.position, CameraPivot.transform.position + offset, ref vel , smoothTime);
        ...
        */
        ...
        if (Inwall())//预测会被遮挡
        {
            ...
            cameraTargetPosition = CameraPivot.transform.position + (wallHit - CameraPivot.transform.position) * 0.8f;

                        
            transform.position = CameraPivot.transform.position + (wallHit - CameraPivot.transform.position) * 0.8f;
            //transform.position = Vector3.SmoothDamp(transform.position, cameraTargetPosition, ref vel, 0.05f);
        ...
        {
            cameraTargetPosition = CameraPivot.transform.position + offset;


            transform.position = CameraPivot.transform.position + offset;
            //transform.position = Vector3.SmoothDamp(transform.position, cameraTargetPosition, ref vel, smoothTime);
